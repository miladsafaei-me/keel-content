  ...
  properties: {
    ...
    brief: {
      ...
      required: ['user_problem', 'intent_statement', 'answer_strategy', 'essential_elements',
                 'complementary_elements', 'glossary_targets', 'visual_obligations', 'visual_gap',
                 'keyword_usage', 'headings_outline',
      ...
      properties: {
        ...
        visual_obligations: { type: 'array',
          description: 'The 0-4 concepts this intent OWES the reader SHOWN, each stated as a format-agnostic JOB (never a component/table/chart name). The downstream visual stage reads these to satisfy intent better than competitors; it decides the actual format from the finished article. [] is valid when prose alone serves the intent.',
          items: { type: 'object', additionalProperties: false, required: ['job', 'why'],
            properties: { job: { type: 'string' }, why: { type: 'string' } } } },
        visual_gap: { type: 'string',
          description: 'One sentence: the thing NO competitor visualizes that this reader needs (the beat-them opportunity), from the evidence read. "" when the evidence shows no gap.' },

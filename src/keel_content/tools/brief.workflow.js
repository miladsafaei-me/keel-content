export const meta = {
  name: 'brief-worklist',
  description:
    'Source-agnostic brief stage: a CLUSTER PASS first divides the need-space between siblings (element ownership, scope fences, link-terms — skipped when the cluster already carries a brief), then one independent agent per article crawls the stored evidence URLs (spec.competitor_urls; WebSearch only as fallback), writes an intent-first structured brief, and an adversarial JUDGE reviews it against the same contract (one revision round). Returns {cluster_briefs, briefs} — the session writes it to a file and binds it with `manage.py contentplan_set_brief`.',
  phases: [
    { title: 'Cluster pass', detail: 'one agent per un-briefed cluster: element ownership + scope fences + link-terms' },
    { title: 'Brief', detail: 'one agent per article: crawl stored evidence -> intent-first brief + feasibility' },
  ...
  properties: {
    ...
    brief: {
      ...
      required: ['user_problem', 'intent_statement', 'answer_strategy', 'essential_elements',
                 'complementary_elements', 'keyword_usage', 'headings_outline',
...
const CLUSTER_SCHEMA = {
  ...
  properties: {
    ...
    cluster_brief: {
      type: 'object',
      additionalProperties: false,
      required: ['shared_context', 'element_ownership', 'scope_fences', 'link_terms', 'notes'],
      properties: {
...
log(`brief stage done: ${briefs.length} brief(s), ${passed} judge-passed ` +
    ...
    'Write {cluster_briefs, briefs} to briefs.json and bind it: manage.py contentplan_set_brief briefs.json')

return { cluster_briefs: clusterBriefsOut, briefs }
